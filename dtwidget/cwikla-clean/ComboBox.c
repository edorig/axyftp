...
static void cancelPopup(DtComboBoxWidget combo) ;
...
Cardinal *num_args;
{
    ...
    combo_p->shell = XtCreatePopupShell("_comboBoxPopup", 
					topLevelShellWidgetClass,
					(Widget)new, args, n);

    XtAddEventHandler(combo_p->shell, ButtonReleaseMask, FALSE, 
	(XtEventHandler)grabEH, (XtPointer)new);
...
Boolean force_label_string;
{
    ...
    if (oldItem) XmStringFree(oldItem);
...
static void grabEH(Widget _w, XtPointer _data, XEvent *_event, Boolean *_cont)
{
    int x, y;
    Window unused;
    DtComboBoxWidget cbw = (DtComboBoxWidget)_data;
...
void DtComboBoxDeselectAllItems(combo)
DtComboBoxWidget combo;
{
	DtComboBoxPart *combo_p = (DtComboBoxPart*)&(combo->combo_box);
...
void
DtComboBoxSelectItemPos(combo, pos)
DtComboBoxWidget combo;
int pos;
{
	DtComboBoxPart *combo_p = (DtComboBoxPart*)&(combo->combo_box);
...
int DtComboBoxGetSelectedPosition(Widget w)
{
...
char *DtComboBoxGetTextString(Widget w)
{
	DtComboBoxWidget combo = (DtComboBoxWidget)w;
...
XmString DtComboBoxGetLabelXmString(Widget w)
{
	DtComboBoxWidget combo = (DtComboBoxWidget)w;
...
void DtComboBoxSetTextString(Widget w, char *_text)
{
	DtComboBoxWidget combo = (DtComboBoxWidget)w;
...
int DtComboBoxItemCount(Widget w)
{
	DtComboBoxWidget combo = (DtComboBoxWidget)w;
	DtComboBoxPart *combo_p = (DtComboBoxPart*)&(combo->combo_box);
...
Widget DtComboBoxGetList(Widget _w)
{
	DtComboBoxWidget combo = (DtComboBoxWidget)_w;
	DtComboBoxPart *combo_p = (DtComboBoxPart*)&(combo->combo_box);

	return combo_p->list;
}

XmString DtComboBoxGetXmStringAtPos(Widget _w, int _pos)
{
	DtComboBoxWidget combo;
	DtComboBoxPart *combo_p;
...
void DtComboBoxReplaceItemAtPos(Widget _w, XmString _item, int _pos)
{
    DtComboBoxWidget combo;
    DtComboBoxPart *combo_p;
...
Widget DtComboBoxGetTextField(Widget _w)
{
	DtComboBoxWidget cbw;

	if (!DtIsComboBox(_w))
		return NULL;

	cbw = (DtComboBoxWidget)_w;
