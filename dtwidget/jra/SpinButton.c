 ...
 * Waltham, MA  02154
 *
 * SpinButton.c (XmSpinButtonWidget):
 *
...
/*
 * 11/99: JRA - Made a few bug fixes and added a couple of minor utilites.
 * adair@iglou.com
 */
#include<stdio.h>
...
static void	Initialize AA((XmSpinButtonWidget request, 
			       XmSpinButtonWidget new, ArgList given_args, 
			       Cardinal *num_args));
static XmNavigability WidgetNavigable AA((XmSpinButtonWidget spin));
static void	_SpinButtonFocusIn AA((XmSpinButtonWidget spin, XEvent *event, 
				       char **params, Cardinal *num_params));
static void	_SpinButtonFocusOut AA((XmSpinButtonWidget spin, XEvent *event,
					char **params, Cardinal *num_params));
static void	DrawHighlight AA((XmSpinButtonWidget spin, Boolean clear));
static void	_SpinButtonUp AA((XmSpinButtonWidget spin, 
				  XEvent *event, char **params, 
				  Cardinal *num_params));
static void	_SpinButtonDown AA((XmSpinButtonWidget spin, 
				    XEvent *event, char **params, 
				    Cardinal *num_params));
static void	_SpinButtonLeft AA((XmSpinButtonWidget spin, 
				    XEvent *event, char **params, 
				    Cardinal *num_params));
static void	_SpinButtonRight AA((XmSpinButtonWidget spin, 
				     XEvent *event, char **params, 
				     Cardinal *num_params));
static void	_SpinButtonBeginLine AA((XmSpinButtonWidget spin, 
					 XEvent *event, char **params, 
					 Cardinal *num_params));
static void	_SpinButtonEndLine AA((XmSpinButtonWidget spin, 
				       XEvent *event, char **params, 
				       Cardinal *num_params));
static void	CheckResources AA((XmSpinButtonWidget spin));
static void	Destroy AA((XmSpinButtonWidget spin));
static void	Resize AA((XmSpinButtonWidget spin));
static void	Redisplay AA((XmSpinButtonWidget w, XEvent *event, 
			      Region region));
static XtGeometryResult GeometryManager AA((Widget w, 
					    XtWidgetGeometry *request, 
					    XtWidgetGeometry *reply));
static void	SetSpinButtonSize AA((XmSpinButtonWidget spin));
static void	ForceChildSizes AA((XmSpinButtonWidget spin));
static void	LayoutChildren AA((XmSpinButtonWidget spin));
static Boolean	SetValues AA((XmSpinButtonWidget current, 
			      XmSpinButtonWidget request, 
			      XmSpinButtonWidget new));
static void	ClearShadow AA((XmSpinButtonWidget w, Boolean all));
static void	DrawShadow AA((XmSpinButtonWidget w));
static void	StoreResourceInfo AA((XmSpinButtonPart *spin_p,
				      XmSpinButtonPart *old_p,
				      Boolean do_items));
static char*	GetTextString AA((XmString xm_string));
static void	SetTextFieldData AA((XmSpinButtonWidget spin));
static void	SetMaximumLabelSize AA((XmSpinButtonPart *spin_p));
static void	SetLabelData AA((XmSpinButtonWidget spin));
static void	timer_dispatch AA((XtPointer client_data, XtIntervalId *id));
static void	TextFieldActivate AA((XmSpinButtonPart *spin_p));
static Boolean	SendCallback AA((XmSpinButtonWidget spin, XEvent *event,
				 Boolean value_changed, int position,
				 float current, Boolean crossed));
static void	FinishUpDown AA((XmSpinButtonWidget spin, 
...
/*
 * 11/99: JRA - Added horizontal and vertical spacing stuff
 */
#define SPIN_H_SPACING(w)   w->spin_button.horizontal_spacing
#define SPIN_V_SPACING(w)   w->spin_button.vertical_spacing
...
/* XmSpinButtonWidget resources */
#define offset(field) XtOffset(XmSpinButtonWidget, field)
static XtResource resources[] = {
    ...
    {XmNhorizontalSpacing, XmCHorizontalSpacing, XmRHorizontalDimension,
     sizeof(Dimension), offset(spin_button.horizontal_spacing),
    ...
    /*
     * 11/99: JRA - Added label font list
     */
    {XmNspinButtonLabelFontList, XmCSpinButtonLabelFontList, XmRFontList,
     sizeof(XmFontList), offset(spin_button.label_font_list), XmRImmediate,
     (XtPointer)NULL},
    ...
    /*
     * 11/99: JRA - Added text font list
     */
    {XmNspinButtonTextFontList, XmCSpinButtonTextFontList, XmRFontList,
     sizeof(XmFontList), offset(spin_button.text_font_list), XmRImmediate,
     (XtPointer)NULL},
    ...
    {XmNverticalSpacing, XmCVerticalSpacing, XmRVerticalDimension,
     sizeof(Dimension), offset(spin_button.vertical_spacing),
     XmRImmediate, (XtPointer)INVALID_DIMENSION},
...
static XmSyntheticResource syn_resources[] = {
    {XmNarrowSize, sizeof(Dimension), offset(spin_button.arrow_size), 
	 _XmSpinButtonGetArrowSize, _XmSetSyntheticResForChild},
};
#undef offset
...
XmSpinButtonClassRec xmSpinButtonClassRec =
{
    ...
    (String)"XmSpinButton",			/* class_name         */    
    (Cardinal)sizeof(XmSpinButtonRec),		/* widget_size        */    
...
Cardinal *num_args;
{
    XmSpinButtonPart *spin_p = (XmSpinButtonPart*)&(new->spin_button);
    /*Widget parent;*/
    char *widget_name;
    Arg args[20]/*, u_args[10], d_args[10]*/;
    int n/*, up_n, down_n*/;
	...
	XtSetArg(args[n], XmNmarginHeight, 2); n++;
         ...
         * 11/99: JRA - Added text font list
         */
        if( spin_p->text_font_list )
        {
           XtSetArg( args[ n ], XmNfontList, spin_p->text_font_list ); n++;

        } /* end if user specified a font list for the text field */
	...
	XtOverrideTranslations((Widget)spin_p->text, child_trans);
        /*
         * 11/99: JRA - Added horizontal and vertical spacing stuff
         */
        if( spin_p->horizontal_spacing == INVALID_DIMENSION )
            spin_p->horizontal_spacing = 0;
        if( spin_p->vertical_spacing == INVALID_DIMENSION )
            spin_p->vertical_spacing = 0;
	...
	XtSetArg(args[n], XmNmarginHeight, 0); n++;
        ...
        * 11/99: JRA - Added label font list
        */
        if( spin_p->label_font_list )
        {
           XtSetArg( args[ n ], XmNfontList, spin_p->label_font_list ); n++;

        } /* end if user specified a font list for the label */
	...
	XtOverrideTranslations((Widget)spin_p->label, child_trans);
         ...
         */
        if( spin_p->horizontal_spacing == INVALID_DIMENSION )
            spin_p->horizontal_spacing = 1;
        if( spin_p->vertical_spacing == INVALID_DIMENSION )
            spin_p->vertical_spacing = 2;
...
Cardinal *num_params;
{
	...
	spin = (XmSpinButtonWidget)XtParent(spin);
...
Cardinal *num_params;
{
    XmSpinButtonPart *spin_p;
    int new_position;
    float new_current;
    
    if (*num_params != 0) /* params means label or arrows */
	spin = (XmSpinButtonWidget)XtParent(spin);
    spin_p = (XmSpinButtonPart*)&(spin->spin_button);
...
XmSpinButtonWidget spin;
{
    XmSpinButtonPart *spin_p = (XmSpinButtonPart*)&(spin->spin_button);
    ...
    /*
    if (spin_p->margin_height < 0) {
	XtWarning(SPIN_MARGIN_HEIGHT);
	spin_p->margin_height = 2;
    }
    if (spin_p->margin_width < 0) {
	XtWarning(SPIN_MARGIN_WIDTH);
	spin_p->margin_width = 2;
    }
    */
    /*
     * 11/99: JRA - Added horizontal and vertical spacing stuff
     */
    /*
    if (spin_p->horizontal_spacing < 0) {
        XtWarning(SPIN_HORIZONTAL_SPACING);
        spin_p->horizontal_spacing = 0;
    }
    if (spin_p->vertical_spacing < 0) {
        XtWarning(SPIN_VERTICAL_SPACING);
        spin_p->vertical_spacing = 0;
    }
    */
...
XtWidgetGeometry *reply;
{
    XmSpinButtonWidget spin = (XmSpinButtonWidget)(w->core.parent);
...
XmSpinButtonWidget spin;
{
     ...
     * 11/99: JRA - Added horizontal and vertical spacing stuff
     */
    Dimension h_spacing = SPIN_H_SPACING(spin) * 2;
    Dimension v_spacing = SPIN_V_SPACING(spin) * 2;
    ...
    (void)XtMakeResizeRequest((Widget)spin, arrow_width +
			      ...
			      height + shadow +v_spacing +
                              (SPIN_MARGIN_H(spin) * 2), 
			      NULL, NULL);
    spin_p->old_width = spin->core.width;
    spin_p->old_height = spin->core.height;
}
...
XmSpinButtonWidget spin;
{
    ...
    if ((available_height = spin->core.height - (SPIN_SHADOW(spin) * 2) - 
	 (SPIN_MARGIN_H(spin) * 2) - (SPIN_V_SPACING(spin) * 2)) <= 0)
	available_height = 1;
    ...
    available_width = (spin->core.width - (SPIN_SHADOW(spin) * 2) - 
		       (SPIN_MARGIN_W(spin) * 2) - (SPIN_H_SPACING(spin) * 2));
...
XmSpinButtonWidget spin;
{
    ...
    XmSpinButtonPart *spin_p = (XmSpinButtonPart*)&(spin->spin_button);
    Widget text_holder = spin_p->editable ? spin_p->text : spin_p->label;
    Position start_x = SPIN_SHADOW(spin) + SPIN_MARGIN_W(spin) + SPIN_H_SPACING(spin);
    Position start_y = SPIN_SHADOW(spin) + SPIN_MARGIN_H(spin) + SPIN_V_SPACING(spin);
...
XmSpinButtonWidget current;
XmSpinButtonWidget request;
XmSpinButtonWidget new;
{
    XmSpinButtonPart *new_p = (XmSpinButtonPart*)&(new->spin_button);
    XmSpinButtonPart *cur_p = (XmSpinButtonPart*)&(current->spin_button);
    Boolean store_info;
    char *widget_name;
    ...
    * 11/99: JRA - Added text font list
    */
   if( new_p->text_font_list != cur_p->text_font_list )
   {
      XtSetArg( args[ n ], XmNfontList, new_p->text_font_list ); n++;

   } /* end if new font list != old font list for text field */

   /*
    * 11/99: JRA - Added label font list
    */
   if( new_p->label_font_list != cur_p->label_font_list )
   {
      XtSetArg( args[ n ], XmNfontList, new_p->label_font_list ); n++;

   } /* end if new font list != old font list for label */
	...
	(SPIN_MARGIN_H(new) != SPIN_MARGIN_H(current)) ||
        (SPIN_H_SPACING( new ) != SPIN_H_SPACING( current ) ) ||
        (SPIN_V_SPACING( new ) != SPIN_V_SPACING( current ) ) ||
...
XmSpinButtonWidget spin;
{
	...
	if (spin_p->items == NULL){
	    ...
	    if ((text = GetTextString(xm_string))!=NULL) {
...
XtIntervalId *id;
{
    XmSpinButtonWidget spin_w = (XmSpinButtonWidget)client_data;
    XmSpinButtonPart *spin_p = (XmSpinButtonPart*)&(spin_w->spin_button);
...
Boolean crossed;
{
    ...
    /* Try to get Xevent */
    /* FIXME is it = or == ????? */
    if ((arrow_data = (XmArrowButtonCallbackStruct*)arrow_call_data)!=NULL)
...
XtPointer call_data;
{
    XmSpinButtonWidget spin = (XmSpinButtonWidget)client_data;
    XmSpinButtonPart *spin_p = (XmSpinButtonPart*)&(spin->spin_button);
 ...
 * with XDesigner a little easier.
 *
 * 11/99: JRA - Modified to check for correct values and if the
 *        user entered a correct value, set the current and
 *        position variables to match it.
 */
static void   text_activate_cb( w, client_data, call_data )
Widget      w;
XtPointer   client_data;
XtPointer   call_data;
{
   ...
   switch( spin_p->child_type )
   {
      /*
       * 11/99: JRA - If it's a string spin button, try to add it via
       * AddItem (with the new uniqueness feature we've added).  If we
       * can't do that, warn user via the bell.
       */
      case XmSTRING:
         str = XmStringCreateLocalized( XmSpinButtonGetTextString( ( Widget ) spin ) );
         if( !XmSpinButtonAddItem( spin, str, spin_p->position + 1, TRUE ) )
         {
            XBell( XtDisplay( spin ), 0 );
            XmStringFree( str );
            return;

         } /* end if we weren't able to add item to the list */

         XmStringFree( str );
         break;
      /*
       * 11/99: JRA - If it's a numeric spin button, get the text, the
       * integer value of the text, (the flot value if it's a float type
       * button), and see if it's within the min and max.  If not, warn user
       * via the bell.  If so, set the private members accordingly.
       * We could potentially add error checking on the atoi/atof, but
       * I'll leave that for now.
       */
      case XmNUMERIC:
         text     = XmTextFieldGetString( spin_p->text );
         intVal   = atoi( text );
         if( spin_p->decimal_points > 0 )
            floatVal = atof( text );
         else
         ...
         if( ( ( spin_p->decimal_points == 0    )   &&
               ( intVal   < spin_p->minimum  ||
                 intVal   > spin_p->maximum     )      )  ||
             ( ( spin_p->decimal_points > 0     )   &&
               ( floatVal < spin_p->min      ||
                 floatVal > spin_p->max         )      )     )
         {
         ...
         } /* end if value entered is not within the allowable range */

         spin_p->current  = floatVal;
         spin_p->position = intVal;

         XtFree( text );
         break;

   } /* end switch on child type */
...
void
_XmSpinButtonGetArrowSize(w, resource_offset, value)
Widget w;
int resource_offset;
XtArgVal *value;
{
    XmSpinButtonWidget spin = (XmSpinButtonWidget)w;
...
Widget 
XmCreateSpinButton(parent, name, arglist, num_args)
Widget parent;
char *name;
Arg *arglist;
int num_args;
{
    return(XtCreateWidget(name, xmSpinButtonWidgetClass, parent,
			  arglist, num_args));
}

/*
 * 11/99: JRA - Modified to add uniqueness to string list items.
 */
Boolean   XmSpinButtonAddItem( spin, item, pos, unique )
XmSpinButtonWidget   spin;
XmString             item;
int                  pos;
Boolean              unique;
   ...
   if( item && total_items )
   {
      for( i=0 ; i<spin_p->item_count ; i++ )
         if( XmStringCompare( item, spin_p->items[ i ] ) )
            break;
      if( ( i < spin_p->item_count ) && unique )
         return( FALSE );

   } /* end if there is an item and a list of items */
...
} /* end XmSpinButtonAddItem() */



void
XmSpinButtonDeletePos(spin, pos)
XmSpinButtonWidget spin;
int pos;
{
    XmSpinButtonPart *spin_p = (XmSpinButtonPart*)&(spin->spin_button);
...
/*
 * 11/99: JRA - Added to get the text in the Text Field widget.
 */
char  *XmSpinButtonGetTextString( Widget widget )
{
   XmSpinButtonWidget   spin;
   XmSpinButtonPart    *spin_p;


   if( !XmIsSpinButton( widget ) )
      return( ( char * ) NULL );

   spin    = ( XmSpinButtonWidget ) widget;
   spin_p  = ( XmSpinButtonPart * ) &( spin->spin_button );

   if( spin_p->editable )
      return( XmTextFieldGetString( spin_p->text ) );
   else
      return( ( char * ) NULL );

} /* end XmSpinButtonGetTextString() */


/*
 * 11/99: JRA - Added to set the background color of the Text Field widget.
 */
void   XmSpinButtonSetTextFieldBackground( widget, bg )
Widget   widget;
Pixel    bg;
{
   XmSpinButtonWidget spin  = ( XmSpinButtonWidget ) widget;
   XmSpinButtonPart *spin_p = ( XmSpinButtonPart * ) &( spin->spin_button );

   if( !spin_p->editable )
      return;

   XtVaSetValues( spin_p->text, XmNbackground, bg, NULL );

} /* end XmSpinButtonSetTextFieldBackground() */


/*
 * 11/99: JRA - Added to get the background color of the Text Field widget.
 */
void   XmSpinButtonGetTextFieldBackground( widget, bg )
Widget   widget;
Pixel   *bg;
{
   XmSpinButtonWidget spin  = ( XmSpinButtonWidget ) widget;
   ...
   XtVaGetValues( spin_p->text, XmNbackground, bg, NULL );

} /* end XmSpinButtonGetTextFieldBackground() */
